...
class RRDBNet(BaseModule):
    ...
    def forward(self, x):
        ...
        ##############################################################
        lq_clean = x.detach().clone()

        for _ in range(0, 3):  # at most 3 cleaning, determined empirically
            residues = self.image_cleaning(lq_clean)
            lq_clean = lq_clean + residues

            # determine whether to continue cleaning
            if torch.mean(torch.abs(residues)) < 1.0:
                break

        feat_ = self.feat_extract(x)
        feat_clean = self.feat_extract_clean(lq_clean)

        feat = torch.cat([feat_, feat_clean], dim=1)
        ...
        return out * 0.2 + self.img_upsample(lq_clean, x)
